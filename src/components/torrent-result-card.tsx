...
function TorrentMetric({ icon: Icon, label, value }: TorrentMetricProps) {
  return (
    <div className="w-16">
      <p className="font-semibold text-foreground-strong">{value}</p>
        ...
        <CardContent className="contents p-0">
          <div className="min-w-0">
            <div className="mb-1 flex items-center gap-1.5 pl-[5px] md:hidden">
              <span className="rounded-md bg-surface-badge px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-[0.1em] text-text-subtle md:px-2 md:py-1 md:text-xs md:tracking-[0.12em]">
                {result.category}
              </span>
              <span className="rounded-md bg-surface-badge px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-[0.1em] text-text-subtle md:px-2 md:py-1 md:text-xs md:tracking-[0.12em]">
                {provider}
            ...
            <div className="flex min-w-0 items-center gap-2 pl-[5px] md:mb-3 md:pl-0">
              <h2 className="min-w-0 flex-1 truncate text-[12px] font-medium leading-5 text-foreground-strong transition-colors group-hover:text-foreground md:text-base md:leading-6">
                {result.title}
              </h2>
            </div>
          ...
          <div className="flex items-end justify-between gap-4 pl-[5px] pr-[5px] md:justify-end md:pl-0 md:pr-0">
            ...
            <span className="shrink-0 pb-0.5 text-right text-[10px] leading-4 text-muted-foreground md:hidden">
